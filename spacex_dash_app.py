...
# Load data
spacex_launch_data = pd.read_csv("https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-DS0321EN-SkillsNetwork/datasets/spacex_launch_dash.csv", encoding="ISO-8859-1")
...
def get_pie_chart(entered_site):
    filtered_df = spacex_launch_data
    if entered_site == 'ALL':
...
def get_scatter(entered_site, payload):
    filtered_df = spacex_launch_data[spacex_launch_data['Payload Mass (kg)'].between(payload[0], payload[1])]
...
# Run the app
# Couldn't get the app to run on port 8050 so manually changed
